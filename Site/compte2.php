...
include_once 'db.php';
$re="SELECT nomCandidat,prenomCandidat FROM Candidat WHERE emailCandidat=?";
$stt=$CONNEXION->prepare($re);
$stt->execute([$_SESSION['email']]);
$result=$stt->fetchAll(PDO::FETCH_ASSOC);
include_once "Actions/securityAction.php";


$r="SELECT * FROM Demande WHERE  candidat=?";
$st=$CONNEXION->prepare($r);
...
<br>
    ...
    <!-- section -->
        ...
        <section class=" ">
            <div class="container  ">
                <div class="row border shadow p-2  justify-content-center">
                  <div class="col-md-6">
                    <?php foreach ($result as $row):;?>
                    <h2 >Bienvenue dans votre compte  <?php echo $row['nomCandidat'];?>   <?php echo $row['prenomCandidat'];?></h2>
                    <?php endforeach;?>
                    <p class="text-success">Félicitations Votre demande est complete !</p>
                    ...
                    <div class="container n">
                        ...
                        <?php  foreach($resultat as $ligne):;?>
                            <div class="alert x alert-info" role="alert">
                              <h2>L'etat de votre demande:</h2>
                                <?php echo $ligne['etat'];?>
                            </div>
                        <?php  endforeach;?>
                       
                        <!-- <a href="demande.php">
                            <button type="submit" class="btn btn-primary text-center">Déposer ma demande se stage</button>
                        </a>       -->
                      </div>
